...
class HeatmapService
{
    ...
    public function computePanels(string $palette, string $timezone = 'UTC'): array
    {
        ...
        if ($scan['count'] === 0) {
            return [$this->emptyPanel('Throughput (Upload x Download)', $cfg),
                $this->emptyPanel('Download x Time of Day', $cfg)];
        }
        ...
        $histUD = $this->histUploadDownload($bins, $bins, $domMinX, $stepX, $domMinY, $stepY);
        $densityUD = $this->adjacentFeather($histUD, $cfg['feather_center'], $cfg['feather_neighbor'], $cfg['feather_diag']);
        ...
        $panelA = [
            ...
            'tiles' => $this->buildTiles(
                $histUD, $densityUD, $bins, $bins, $xLabels, $yLabelsUp,
                $clampMinUD, $clampMaxUD, $palette, $cfg['value_scale'],
        ...
        $histDH = $this->histDownloadByHour($cols, $rows, $stepH, $domMinY, $stepY, $timezone);
        $densityDH = $this->adjacentFeather($histDH, $cfg['feather_center'], $cfg['feather_neighbor'], $cfg['feather_diag']);
        ...
        $panelB = [
            ...
            'gap' => $cfg['gap'],
            'tiles' => $this->buildTiles(
                $histDH, $densityDH, $cols, $rows, $xLabelsH, $yLabelsUpH,
                $clampMinDH, $clampMaxDH, $palette, $cfg['value_scale'],
                $cfg['tile'], $cfg['gap'], $cfg['color_empty']
            ),
            'legendBar' => $this->legendBar($palette, $cfg['legend_steps']),
    ...
    {
        $this->baseQuery()->chunkById(5000, function ($chunk) use ($cb) {
            foreach ($chunk as $r) {
                $db = $r->download_bits ?? (isset($r->download) ? (float) $r->download * 8.0 : null);
                $ub = $r->upload_bits ?? (isset($r->upload) ? (float) $r->upload * 8.0 : null);
                if ($db === null || $ub === null) {
                    continue;
                }
    ...
    private function histUploadDownload(int $cols, int $rows, float $domMinX, float $stepX, float $domMinY, float $stepY): array
    {
    ...
    private function histDownloadByHour(int $cols, int $rows, float $stepHour, float $domMinY, float $stepY, string $timezone): array
    {
        $hist = array_fill(0, $rows, array_fill(0, $cols, 0.0));
        $this->streamResults(function ($_up, float $dn, $ts) use (&$hist, $cols, $rows, $stepHour, $domMinY, $stepY, $timezone) {
            if (! $ts) {
                return;
            }
    ...
    private function buildTiles(array $hist, array $density, int $cols, int $rows, array $xLabels, array $yLabelsUp,
        float $clampMin, float $clampMax, string $palette, string $valueScale,
        int $tile, int $gap, bool $colorEmpty): array
    {
        $tiles = [];
        for ($yy = 0; $yy < $rows; $yy++) {
            for ($xx = 0; $xx < $cols; $xx++) {
    ...
    private function domainWithHeadroom(float $min, float $max, ?float $hardMin, ?float $hardMax, float $headroom): array
    {
        ...
        if ($hardMin !== null) {
            $rawMin = min($rawMin, $hardMin);
        }
        if ($hardMax !== null) {
            $rawMax = max($rawMax, $hardMax);
        }
    ...
    private function legendClamp(array $density, string $mode): array
    {
        ...
        switch ($mode) {
            case 'max': $hi = $maxBin;
                break;
            case 'p95': $hi = max(1.0, $p95);
                break;
            case 'p99': $hi = max(1.0, $p99);
                break;
            default:
                $hi = max(1.0, $p99);
                if ($maxBin > 3 * $hi) {
    ...
    private function scaleValue(float $v, float $min, float $max, string $mode): float
    {
        $v = max($min, min($max, $v));
        switch ($mode) {
            case 'log': $t = log(1 + $v) / max(log(1 + $max), 1e-9);
                break;
            case 'sqrt': $t = sqrt($v / max($max, 1e-9));
                break;
            default: $t = ($v - $min) / max($max - $min, 1e-9);
                break;
        }
